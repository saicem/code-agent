...
async def run_sub_agent_task(
    params: SubAgentTaskParams, session_manager: SessionManager = Provide[Container.session_manager]
) -> str:
    from code_agent.agent.engine import reasoning_acting

    if not params.task:
        return build_tool_response(False, "任务描述不能为空")

    print_tool_output(f"委托子任务: {params.task}", "delegate_to_sub_agent")
    sub_session = Session()
    sub_session.set_system_prompt(CODE_SYSTEM)
    sub_session.add_user_message(params.task)
